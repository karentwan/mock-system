...
JAVA_OPTS=" -Djava.awt.headless=true -Djava.net.preferIPv4Stack=true -Dspring.main.allow-bean-definition-overriding=true"
...
if [ "$1" == "stdout" ]; then
  nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $SENTINEL_OPTS -Xbootclasspath/a:$CONF_DIR -Xbootclasspath/a:$PLUGIN_DIR -DlogPath=$LOGPATH -Dspring.profiles.active=$PROFILES_ACTIVE -jar $MAIN_JAR >>$STDOUT_FILE 2>&1 &
else
  nohup java $JAVA_OPTS $JAVA_MEM_OPTS $JAVA_DEBUG_OPTS $JAVA_JMX_OPTS $SENTINEL_OPTS -Xbootclasspath/a:$CONF_DIR -Xbootclasspath/a:$PLUGIN_DIR -DlogPath=$LOGPATH -Dspring.profiles.active=$PROFILES_ACTIVE -jar $MAIN_JAR >>/dev/null 2>&1 &
fi
